...
class advancedSearchResult extends Component {
  ...
  render() {
    ...
    this.state.resAliment.map((el, index) => {
      if (index !== 0) {
        elementStr +=
          "<div className='aliment'>\n <div>" +
          el.product_name +
          "</div>\n <div>Nutrition : " +
          el.nutrition_data_per +
          "</div> \n<div>quantity : " +
          el.quantity +
          "</div> \n</div>";
      }
      ...
      if (index === this.state.resAliment.length - 1) {
        elementStr +=
          "<div className='aliment'>\n <div>" +
          this.state.resAliment[0].product_name +
          "</div>\n <div>Nutrition : " +
          this.state.resAliment[0].nutrition_data_per +
          "</div> \n<div>quantity : " +
          this.state.resAliment[0].quantity +
          "</div> \n</div> \n</div>";
      }
    });

    let insideContent = <Container>{parse(elementStr)}</Container>;
